...
void pMain(in V2P i, out float4 oColor : SV_Target) {
	...
	float4 col = 0;
	col += blitTex.SampleLevel(samplerState, i.uv, 0);
	...
	col += blitTex.SampleLevel(samplerState, i.uv + float2(off.x, 0), 0);
	col += blitTex.SampleLevel(samplerState, i.uv + float2(off.z, 0), 0);
	col += blitTex.SampleLevel(samplerState, i.uv + float2(0, off.y), 0);
	col += blitTex.SampleLevel(samplerState, i.uv + float2(0, off.w), 0);
	oColor = col * (1.0/9.0);
#else
